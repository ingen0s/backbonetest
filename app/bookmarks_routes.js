//TODO - check link for explanation on this use. CRUD REST
...
function handleGet(connection,req,res) {
    ...
    connection.query('SELECT * FROM bookmarks ORDER BY id DESC LIMIT ' + limit, function handleSql(err, rows) {
        if (err){ logAndRespond(err,res); return; }
...
function handleFind(connection,req,res) {
    var find = function find(id){
        connection.query('SELECT * FROM bookmarks WHERE id = ?', id, function handleSql(err, rows) {
            if (err){ logAndRespond(err,res); return; }
            if (rows.length === 0){ res.send(204); return; }
            res.send({
                result: 'success',
...
function handleIns(connection,req,res) {
    connection.query('INSERT INTO bookmarks SET ?', req.body, function handleSql(err, result) {
        if (err){ logAndRespond(err,res); return; }
        res.statusCode = 201;
        res.send({
            result: 'success',
...
function handleUpd(connection,req,res) {
    connection.query('UPDATE bookmarks SET ? WHERE id='+req.params.id, req.query, function handleSql(err) {
        if (err){ logAndRespond(err,res); return; }
